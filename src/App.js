...
class App extends Component {
  ...
  Verifyimg = imageUrl => {
    const {thumimgindex, score} = this.state
    const randomIndex = Math.floor(Math.random() * imagesList.length)
    const upscore = score + 1
    if (imagesList[thumimgindex].imageUrl === imageUrl) {
      this.setState({thumimgindex: randomIndex, score: upscore})
    }
  }

  render() {
    const {thumimgindex, category, score, timer, over} = this.state
    const updatedList = imagesList.filter(each => each.category === category)
    const img = imagesList[thumimgindex].thumbnailUrl
    const firstImageUrl = imagesList[0].imageUrl
    return (
      <div className="App_con">
        {over && <Gameover play={this.onPlayAgain} />}

        {!over && (
          <div>
            <Navbar score={score} timer={timer} />
            <ThumbNail thumbnail={img} />
            <ul className="button_con">
              {tabsList.map(each => (
                <TabList
                  key={each.tabId}
                  displayText={each.displayText}
                  tabId={each.tabId}
                  clickid={this.ChangeTab}
                />
              ))}
            </ul>

            <ul className="image_Con">
              <img alt="match" src={firstImageUrl} />
              {updatedList.map(each => (
                <ItemList
                  key={each.id}
                  category={each.category}
                  imageUrl={each.imageUrl}
                  verify={this.Verifyimg}
                />
              ))}
            </ul>
          </div>
